...
def evaluate_individual_strategies(func_code, original_curve, original_var,max_change_percent=0.03):
    """
    ...
    for name, func in strategy_funcs.items():
        try:
            # 执行策略函数获取deltas
            deltas_mean, deltas_var = func(original_curve,original_var)
            if len(deltas_mean) != 24 or len(deltas_var) != 24:
                print(f"策略 {name} 返回的delta列表长度错误")
                continue
            ...
            adjusted_curve_var = []
            for i in range(24):
                change = deltas_mean[i]
                change_var = deltas_var[i]
                max_change = original_curve[i] * max_change_percent
                if abs(change) > max_change:
                    change = np.sign(change) * max_change
                max_change_var = original_var[i] * max_change_percent
                if abs(change_var) > max_change_var:
                    change_var = np.sign(change_var) * max_change_var
                adjusted_curve.append(original_curve[i] + change)
                adjusted_curve_var.append(original_var[i] + change_var)
            ...
            results[name] = {
                ...
                'adjusted_curve_var': adjusted_curve_var,
                'delta_curve_mean': deltas_mean,
                'delta_curve_var': deltas_var,
                'func_code': func_str
            }
        except Exception as e:
...
def execute_strategy_function(func_code, original_load, original_var, max_change_percent=0.1):
    """
    安全执行模型生成的策略函数
    """
    ...
    restricted_globals = {
            ...
            'np': None,
            'name': 'sandbox' # 关键：防止 if name == "main" 中的代码执行
            }
    ...
    try:
        ...
        # 执行策略函数
        delta_curve, delta_var = strategy_func(original_load, original_var)
        ...
        for i in range(24):
            ...
            adjusted_curve.append(original_load[i] + max(-max_delta, min(delta_curve[i], max_delta)))
            adjusted_var_value.append(original_var[i] + max(-max_delta, min(delta_var[i], max_delta)))
            
        return adjusted_curve,adjusted_var_value, delta_curve, func_code  # 返回原始代码
    
    except Exception as e:
        print(f"策略函数执行错误: {str(e)}")
        # 返回原始曲线和原始代码（即使执行失败也保留代码）
        return original_load,original_var, [0]*24, func_code
