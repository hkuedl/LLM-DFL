...
class Dataset_load(Dataset):
    def __init__(self,  data_path='../Data/GEF_data/data(3 year).csv', flag='train', size=[96,0,24], train_length=4296, target='load', scale=True, inverse=True):
        self.seq_len = size[0]
        self.label_len = size[1]
        self.pred_len = size[2]
        self.data_path = data_path
    ...
    def process_datasets(self):
        self.scaler_x = MinMaxScaler()# 
        self.scaler_y = MinMaxScaler()# 
        self.scaler_x_cal = MinMaxScaler()# 
        data=pd.read_csv(self.data_path)
        data = data.interpolate(method='cubic', limit_direction='both')
        ...
        # 划分训练和测试数据
        train_data = data[0:24*(365+366-8-30)]
        test_data = data[24*(365+366-8-30):24*(365+366-9)]
        ...
        if self.scale:
            ...
            X_train_norm=X_train_norm.reshape((-1,24,len(self.X_features_name)))
            y_train_norm=y_train_norm.reshape(-1,24)
            X_test_norm=X_test_norm.reshape((-1,24,len(self.X_features_name)))
            y_test_norm=y_test_norm.reshape(-1,24) 
            print(np.shape(X_train_norm),np.shape(y_train_norm),np.shape(X_test_norm),np.shape(y_test_norm))
            X_train=X_train_norm[0:366-8]
            y_train=y_train_norm[0:366-8]
            X_val, y_val = X_train_norm[366-8:], y_train_norm[366-8:]
    ...
    def inverse_transform_X(self, X_nor):
        return self.scaler_x.inverse_transform(X_nor.reshape(-1, len(self.X_features_name)))
        ...
        for curve_key, id_list in self.curve_index.items():
            ...
            for record in records:
                ...
                grouped_records[curve_key].append({
                    ...
                    'load': record['load'],
                    'pred_var': record['pred_var'],
                    'fine_tuned_load': record.get('fine_tuned_load', []),
                    'fine_tuned_var': record.get('fine_tuned_var', []),
                    'cost': record['cost'],
