...
class DA_dispatch_SO_cvxpy():
    def __init__(self, args):
        super(DA_dispatch_SO_cvxpy, self).__init__()
        T = args.T
        N_g = args.N_g
        N = args.N
        l_for = cp.Parameter(T, nonneg=True)
        scenarios = cp.Parameter((N, T), nonneg=True)
        ...
        z = cp.Variable((N,2 * N_g * T), nonneg=True)
        r = cp.Variable((N,2*T), nonneg=True)
        ...
        x_max_list = torch.ones(N_g * T, dtype=torch.float64)
        r_pos_list = torch.ones(N_g * (T - 1), dtype=torch.float64)
        r_neg_list = torch.ones(N_g * (T - 1), dtype=torch.float64)
        for i in range(N_g):
                x_max_list[i * T:(i + 1) * T] = x_max[i]
                r_pos_list[i * (T - 1):(i + 1) * (T - 1)] = r_pos[i]
                r_neg_list[i * (T - 1):(i + 1) * (T - 1)] = r_neg[i]
        ...
        for i in range(N):
            error_l = l_for-scenarios[i]
            constraints += [
                F_uq @ z[i] <= z_max,
                F_eq_z @ z[i] + G_eq_l @ (error_l) + F_eq_r @ r[i] == h_eq
            ]

        objective = cp.Minimize(alpha @ x + alpha_z @ z_max+ sum([cp.sum(rho_z @ z[i]) + cp.sum(rho_r @ r[i]) for i in range(N)])/N)

        problem = cp.Problem(objective, constraints)
        self.cvxpyLayer = CvxpyLayer(problem, parameters=[l_for,scenarios, alpha, alpha_z,rho_z,rho_r],
                                     variables=[x, z_max])



class RT_dispatch_cvxpy():
    def __init__(self, args):
        super(RT_dispatch_cvxpy, self).__init__()
        T = args.T
        N = args.N
        N_g = args.N_g
...
class Combined_SO_dispatch_cvxpy(torch.nn.Module):
    def __init__(self, SO_layer,Intra_schdule_layer, model_load):
        super(Combined_SO_dispatch_cvxpy, self).__init__()
        self.model_load = model_load
        self.Deterministic_layer = SO_layer.cvxpyLayer
    ...
    def model_load_forward(self, x, scaler_y):
        if isinstance(scaler_y, StandardScaler):
            ...
            pred_mu_nor, pred_var_nor = self.model_load(x)
            pred_mu = (pred_mu_nor.reshape(-1,1) * scale_torch + mean_torch).reshape(-1,24)
            pred_var = (pred_var_nor.reshape(-1,1) * scale_torch).reshape(-1,24)
            # pred_mu_nor * scale_torch + mean_torch
            # pred_var = pred_var_nor * scale_torch
            ...
            pred_mu_nor, pred_var_nor = self.model_load(x)
            pred_mu = (pred_mu_nor.reshape(-1,1) * (max_torch - min_torch) + min_torch).reshape(-1,24)
            pred_var = (pred_var_nor.reshape(-1,1) * (max_torch - min_torch)).reshape(-1,24)
            # pred_mu_nor * (max_torch - min_torch) + min_torch
            # pred_var = pred_var_nor * (max_torch - min_torch)
        return pred_mu, pred_var
    ...
    def forward(self,args,forecasts_mu,forecasts_var,targets_load,alpha,alpha_z,rho_z,rho_r):
        set_seed(42)
        scenarios_nor = np.random.random(size=(args.N, args.T))  
        z = stats.norm.ppf(scenarios_nor)
        z_torch = torch.tensor(z, dtype=torch.float64).to(args.device)
        # forecasts_mu [7,24], forecasts_var [7,24], z_torch [25, 24],最后要得到7,25,24
        scenarios = z_torch.unsqueeze(0) * forecasts_var.unsqueeze(1)+ forecasts_mu.unsqueeze(1)  
        ...
        while True:
            try:
                solution_ahead = self.Deterministic_layer(forecasts_mu,scenarios, alpha,alpha_z,rho_z,rho_r, solver_args={'solve_method':'ECOS'})
                solution_dict_ahead =self.switch_solution_to_dict_ahead(solution_ahead)
                break
            except Exception as e:
                solution_ahead = self.Deterministic_layer(forecasts_mu,scenarios, alpha,alpha_z,rho_z,rho_r, solver_args={'solve_method':'SCS','max_iters':5000})
                solution_dict_ahead =self.switch_solution_to_dict_ahead(solution_ahead)
                break
        while True:
            try:
                solution_intra=self.Intra_schdule_layer(solution_dict_ahead['z_max'],forecasts_mu, targets_load,rho_z,rho_r, solver_args={'solve_method':'ECOS'})
                solution_dict_intra =self.switch_solution_to_dict_intra(solution_intra)
                break   
            except Exception as e:
                solution_intra=self.Intra_schdule_layer(solution_dict_ahead['z_max'],forecasts_mu, targets_load,rho_z,rho_r, solver_args={'solve_method':'SCS','max_iters':5000})
                solution_dict_intra =self.switch_solution_to_dict_intra(solution_intra)
                break    
...
class DA_dispatch_SO_RGB:
    ...
    def _precompute_matrices(self):
        ...
        x_max_list = np.ones(N_g * T)
        r_pos_list = np.ones(N_g * (T - 1))
        r_neg_list = np.ones(N_g * (T - 1))
        for i in range(N_g):
                x_max_list[i * T:(i + 1) * T] = self.x_max[i]
                r_pos_list[i * (T - 1):(i + 1) * (T - 1)] = self.r_pos[i]
                r_neg_list[i * (T - 1):(i + 1) * (T - 1)] = self.r_neg[i]
    ...
    def solve(self, l_for, scenarios, alpha, alpha_z,rho_z,rho_r):
        # 转换输入为NumPy数组并确保非负
        ...
        z = model.addMVar((self.N,2 * self.N_g * self.T), lb=0.0, name="z")
        r = model.addMVar((self.N,2*self.T), lb=0.0, name="r")
        ...
        for i in range(self.N):
            ...
            model.addConstr(
                self.F_uq @ z[i] <= z_max,
                name=f"reserve_limit_scenario_{i}"
            )
            
            model.addConstr(
                self.F_eq_z @ z[i] + self.G_eq_l @ error_l + self.F_eq_r @ r[i] == self.h_eq,
                name=f"power_balance_scenario_{i}"
            )

        obj = alpha @ x + alpha_z @ z_max + sum([rho_z @ z[i] + rho_r @ r[i] for i in range(self.N)])/self.N
...
class Combined_SO_dispatch_GRB(torch.nn.Module):
    ...
    def __init__(self, args, alpha, alpha_z, rho_z, rho_r):
        ...
        """
        self.args = args
        self.DA_dispatch_SO_RGB = DA_dispatch_SO_RGB(args)
        self.RT_dispatch_GRB = RT_dispatch_GRB(args)
        self.alpha=alpha
        self.alpha_z=alpha_z
        self.rho_z=rho_z
        self.rho_r=rho_r
    
    def forward(self, forecasts_mu,forecasts_var, targets_load):
        # 转换输入为NumPy数组
        forecasts_load = np.asarray(forecasts_mu, dtype=np.float64).flatten()
        ...
        set_seed(42)
        scenarios_nor = np.random.random(size=(self.args.N, len(forecasts_mu)))  
        z = stats.norm.ppf(scenarios_nor)  # 逆CDF映射到标准正态 [N,24,30]
        scenarios = forecasts_mu + forecasts_var * z  # [N,24,30]
        # 第一阶段：日前调度
        try:
            x, z_max = self.DA_dispatch_SO_RGB.solve(forecasts_load,scenarios, alpha, alpha_z, rho_z, rho_r)
        except Exception as e:
            print(f"日前调度求解失败: {e}")
            raise
        ...
        # 第二阶段：日内调度
        try:
            z, r = self.RT_dispatch_GRB.solve(z_max, forecasts_load, targets_load, rho_z, rho_r)
        except Exception as e:
            print(f"日内调度求解失败: {e}")
            raise
